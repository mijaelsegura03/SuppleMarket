...
async function fillAccordionWithDataAndListeners() {
	const emptyList = await renderPurchases();
	if (!emptyList) {
...
async function renderPurchases() {
  ...
  if (response.status == 404) {
	...
	purchases.sort((purchaseA, purchaseB) => {
		return (
			new Date(purchaseB.purchaseDate) - new Date(purchaseA.purchaseDate)
		);
	});
	let idx = 1;
...
async function renderDetails(purchaseDetail, jwt, idx) {
	...
	for (const detail of purchaseDetail) {
		const response = await fetch(
			`${SUPPLEMENTS_URL}/${detail.supplementId}`,
			{
				headers: {
					Authorization: `Bearer ${jwt}`,
				},
	...
	accordionHeaders.forEach((header) => {
		header.addEventListener("click", () => {
			...
			if (accordionContent.classList.contains("active")) {
				let addedHeight =
					accordionContent.querySelector(".list-item").children
						.length *
						10 +
					20;
				accordionContent.style.maxHeight =
					accordionContent.scrollHeight + addedHeight + "px";
...
function renderEmptyList() {
  ...
  message.textContent = "You didn't make any purchases yet"
  purchasesContainer.appendChild(message)
}
